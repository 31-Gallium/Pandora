...
class TimeHub(BaseHubModule):
    ...
    def _draw_digital(self, p, cx, cy, inner_radius, now, label_text, theme='Dark', accent_color=None):
        ...
        text_color_sub = QColor(36, 41, 47, 140) if is_light else QColor(255, 255, 255, 120)
        ...
        # Subtle Drop Shadow for depth (skip in light mode for cleaner look)
        if not is_light:
    ...
    def _draw_analog(self, p, cx, cy, inner_radius, now, label_text, theme='Dark', accent_color=None):
        ...
        rim_color = QColor(0, 0, 0, 15) if is_light else QColor(255, 255, 255, 10)
        ...
        for i in range(60):
            ...
            color = QColor(36, 41, 47, alpha) if is_light else QColor(255, 255, 255, alpha)
            p.setPen(QPen(color, w, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            p.drawLine(QPointF(x1, y1), QPointF(x2, y2))

        # 3. Timezone label inside analog face
        label_c = QColor(36, 41, 47, 140) if is_light else QColor(255, 255, 255, 140)
        p.setPen(label_c)
        ...
        if show_date:
            ...
            p.setBrush(QColor(0, 0, 0, 40) if not is_light else QColor(0,0,0,15))
            date_rect_shadow = QRectF(cx - 11, cy + r * 0.4 + 1, 22, 16)
            p.drawRoundedRect(date_rect_shadow, 4, 4)
            
            box_pen = QColor(0,0,0,20) if is_light else QColor(255,255,255,20)
            box_bg = QColor(0,0,0,10) if is_light else QColor(255,255,255,10)
            p.setPen(QPen(box_pen, 1))
            p.setBrush(box_bg)
            date_rect = QRectF(cx - 11, cy + r * 0.4, 22, 16)
            p.drawRoundedRect(date_rect, 4, 4)
            
            p.setPen(QColor(base_color.red(), base_color.green(), base_color.blue(), 255))
        ...
        h_len = r * 0.45
        hand_h_c = QColor(36, 41, 47, 240) if is_light else QColor(255, 255, 255, 240)
        draw_rounded_hand(h_angle, h_len, 14, 6.0, hand_h_c)

        # 6. Minute hand (Medium thick rounded line)
        m_angle = math.radians((now.minute + now.second / 60) * 6 - 90)
        m_len = r * 0.70
        hand_m_c = QColor(36, 41, 47, 190) if is_light else QColor(255, 255, 255, 190)
        draw_rounded_hand(m_angle, m_len, 16, 4.0, hand_m_c)
