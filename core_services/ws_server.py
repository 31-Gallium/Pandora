...
class WebSocketServerThread(QThread):
    ...
    extract_halo_icon_requested = pyqtSignal(str, str)
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    elif payload.get('type') == 'extract_halo_icon':
                        self.extract_halo_icon_requested.emit(payload.get('path', ''), payload.get('req_id', ''))
