...
from core_services.audio_engine import AudioEngineClient, AudioFeatures, EngineState
...
class MediaDaemon(QObject):
    ...
    def __init__(self):
        ...
        self._current_engine_pid = -1
        self._engine_target_app_id = ""

        self._thread = threading.Thread(target=self._run_event_loop, daemon=True)
        self._thread.start()

    def _get_target_pid_for_app(self, app_id: str) -> int:
        target = app_id.lower()
        if not target:
            return -1
        try:
    ...
    def _update_audio_engine(self):
        app_id = self.state.app_id
        if not app_id or self.state.status != "Playing":
            self._stop_audio_engine()
            return
            
        if self._engine_target_app_id == app_id and self.audio_engine and self.audio_engine.state == EngineState.CONNECTED:
            return
            
        target_pid = self._get_target_pid_for_app(app_id)
        if target_pid > 0 and target_pid != self._current_engine_pid:
            self._stop_audio_engine()
            self._current_engine_pid = target_pid
            self._engine_target_app_id = app_id
            
            # Start native engine process
            exe_path = os.path.join(os.path.dirname(__file__), "..", "native_engine", "build", "Release", "AudioCaptureService.exe")
            # Fallback to local if packaged
            if not os.path.exists(exe_path):
                exe_path = os.path.join(os.path.dirname(__file__), "..", "AudioCaptureService.exe")
            
            if os.path.exists(exe_path):
                try:
                    # CREATE_NO_WINDOW = 0x08000000
                    self._engine_process = subprocess.Popen([exe_path, str(target_pid)], creationflags=0x08000000)
                    
                    # Start client
                    self.audio_engine = AudioEngineClient(self._engine_process.pid)
                    self.audio_engine.features_updated.connect(self._on_audio_features)
                    self.audio_engine.state_changed.connect(self._on_engine_state_changed)
                    self.audio_engine.start()
                except Exception as e:
                    print(f"[MediaDaemon] Failed to start Audio Engine: {e}")
    ...
    def _stop_audio_engine(self):
        ...
        if self._engine_process:
            try:
                ...
                self._engine_process.wait(timeout=1.0)
            except:
                pass
            self._engine_process = None
        self._current_engine_pid = -1
    ...
    def _on_engine_state_changed(self, state: EngineState):
        self.state.engine_state = state.name.title()
        self._emit_update()

    def _on_audio_features(self, features: AudioFeatures):
    ...
    async def _sync_playback_state(self):
        ...
        try:
            ...
            if status_changed or pos_changed or self.state.can_pause != can_pause:
                ...
                # Check if we should start/stop the audio engine based on playback state
                self._update_audio_engine()
                self._emit_update()
        except: pass
