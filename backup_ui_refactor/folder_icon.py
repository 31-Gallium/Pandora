...
class FolderIcon(QWidget):
    ...
    def _update_movie(self):
        cp = self.data.get('cover_path') or self.data.get('cover_image')
    ...
    def get_setting(self, key, default):
        ...
        use_custom = self.data.get('use_custom', False)
        
        preset = t_data.get('size_preset', 'Medium')
        if use_custom:
            preset = self.data.get('size_preset', preset)
        ...
        if use_custom:
            val = next((self.data[lk] for lk in lookup_keys if lk in self.data), None)
