...
class GridOverlay(QWidget):
    ...
    def get_snap_pos(self, global_pos):
        ...
        edge_padding = gen.get('edge_padding', 0)
        margin = int(40 + edge_padding)
        ...
        snapped_x = max(scr_geom.left() + margin, min(snapped_x, scr_geom.right() - margin))
        snapped_y = max(scr_geom.top() + margin, min(snapped_y, scr_geom.bottom() - margin))
