...
class Halo(QWidget):
    ...
    def _do_paint(self, p):
        ...
        if theme == 'Desktop':
            ...
            self._accent_colors = [QColor(r, g, b) for r, g, b in accents]
        ...
        if getattr(self, 'layer_anim_progress', 0.0) < 0.01:
            for i in range(num_tools):
                ...
                if prog > 0.01:
                    ...
                    if getattr(self, '_accent_colors', None):
                        acs = self._accent_colors
                        n = len(acs)
                        if n > 1:
                            grad = QConicalGradient(cx, cy, 90)
                            for idx, c in enumerate(acs):
                                grad.setColorAt(idx / n, QColor(c.red(), c.green(), c.blue(), slice_alpha))
                            grad.setColorAt(1.0, QColor(acs[0].red(), acs[0].green(), acs[0].blue(), slice_alpha))
                        else:
                            grad = QLinearGradient(cx, cy - r_outer, cx, cy + r_outer)
                            grad.setColorAt(0, QColor(acs[0].red(), acs[0].green(), acs[0].blue(), slice_alpha))
                            grad.setColorAt(1, QColor(min(255, acs[0].red()+50), min(255, acs[0].green()+50), min(255, acs[0].blue()+50), slice_alpha))
                    else:
                        grad = QLinearGradient(cx, cy - r_outer, cx, cy + r_outer)
                        grad.setColorAt(0, QColor(0, 120, 255, slice_alpha))
                        grad.setColorAt(1, QColor(0, 200, 255, slice_alpha))
            ...
            for i in range(num):
                ...
                if pix_size > 0:
                    if tool['id'].startswith('stopwatch_lap_'):
                        ...
                        ic_col_final = "#ffffff" if prog > 0.5 else ("#d0d0d0" if getattr(self, 'theme', 'Dark') == 'Dark' else "#333333")
                        
                        # Request the icon at the maximum size (32) to hit the cache instantly and avoid mid-animation re-renders
                        pix = VectorIcon.pixmap(tool['icon'], ic_col_final, 32)
