...
def handle_app_drop(cfg, target_folder_data, mime_data, e_source, target_idx, dashboard=None):
    """
    ...
    for ad in dropped_apps:
        ...
        # Enforce Max depth = 1: do not allow dropping a folder into a nested folder
        if old_path.startswith('pandora://folder/'):
            print(f">>> [DEBUG] Dropping Pandora folder {old_path} into target {target_folder_data.get('id')} (is_nested: {target_folder_data.get('is_nested')}) <<<")
            logger.error(f"[NESTING-CHECK] Dropping folder {old_path} into target folder: {target_folder_data.get('id')} which has is_nested={target_folder_data.get('is_nested')}")
            if target_folder_data.get('is_nested'):
                print(f">>> [DEBUG] BLOCKED! Target {target_folder_data.get('id')} is already a nested folder! <<<")
                logger.error("[NESTING-CHECK] Blocked! Max nesting depth is 1.")
                return False, []
        ...
        if is_custom:
            ...
            target_folder_data['apps'].append(ad)
