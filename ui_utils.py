...
def draw_folder_thumbnail(p, rect, data, cfg, local_settings=None, hover_progress=0.0, paging_params=None):
    ...
    def get_val(key, default):
        INSTANCE_KEYS = ('show_title', 'grid_snap', 'show_cover', 'title_color', 'cover_image')
        ...
        use_custom = data.get('use_custom_settings', False)
        if use_custom:
            val = data.get('custom_settings', {}).get(key)
        ...
        if use_custom: preset = data.get('custom_settings', {}).get('size_preset', preset)
        SIZE_PRESETS = {
        ...
        if preset in SIZE_PRESETS and key in SIZE_PRESETS[preset]: return SIZE_PRESETS[preset][key]
    ...
    # Draw Cover
    cover_path = data.get('cover_image')
    if show_cover:
