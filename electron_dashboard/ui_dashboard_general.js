...
function applyDashboardTheme(cfg) {
    ...
    if (t === 'Light') {
        ...
        if (accents.length > 0) {
            ...
            // 1. Calculate overall vibe of the wallpaper (average luminance)
            let totalLuma = 0;
            accents.forEach(([r, g, b]) => {
                totalLuma += (r * 0.299 + g * 0.587 + b * 0.114);
            });
            const avgLuma = totalLuma / accents.length;
            const isLightVibe = avgLuma > 130;
            
            if (isLightVibe) {
                document.body.classList.add('light-theme');
            }
            
            // 2. Adjust accents to ensure they pop against their respective background
            let finalAccents = accents.map(([r, g, b]) => {
                const luma = (r * 0.299 + g * 0.587 + b * 0.114);
                if (isLightVibe) {
                    // Darken accents that are too light for a white background
                    if (luma > 140) {
                        const factor = 100 / (luma + 1);
                        return [Math.round(r * factor), Math.round(g * factor), Math.round(b * factor)];
                    }
                } else {
                    // Brighten accents that are too dark for a black background
                    if (luma < 110) {
                        const factor = 140 / (luma + 1);
            ...
            // Sort accents by most contrast for the primary accent
            if (isLightVibe) {
                // Sort darkest first
                finalAccents.sort((a, b) => (a[0]*0.299 + a[1]*0.587 + a[2]*0.114) - (b[0]*0.299 + b[1]*0.587 + b[2]*0.114));
            } else {
                // Sort brightest first
                finalAccents.sort((a, b) => (b[0]*0.299 + b[1]*0.587 + b[2]*0.114) - (a[0]*0.299 + a[1]*0.587 + a[2]*0.114));
            }
            
            finalAccents.forEach((rgb, i) => {
                const [r, g, b] = rgb;
                root.setProperty(`--desktop-accent-${i + 1}`, `rgb(${r},${g},${b})`);
                root.setProperty(`--desktop-accent-${i + 1}-rgb`, `${r}, ${g}, ${b}`);
            });
            
            const [r0, g0, b0] = finalAccents[0];
            root.setProperty('--desktop-glow-1', `rgba(${r0},${g0},${b0},0.15)`);
            
            // 3. Derive surface background colors
            const [origR, origG, origB] = accents[0]; // Use the most abundant original color for background tint
            if (isLightVibe) {
                const sr = Math.round(origR * 0.05 + 240);
                const sg = Math.round(origG * 0.05 + 240);
                const sb = Math.round(origB * 0.05 + 240);
                root.setProperty('--desktop-surface-1', `rgb(${Math.min(sr, 255)},${Math.min(sg, 255)},${Math.min(sb, 255)})`);
                
                const sr2 = Math.round(origR * 0.08 + 230);
                const sg2 = Math.round(origG * 0.08 + 230);
                const sb2 = Math.round(origB * 0.08 + 230);
                root.setProperty('--desktop-surface-2', `rgb(${Math.min(sr2, 255)},${Math.min(sg2, 255)},${Math.min(sb2, 255)})`);
            } else {
                const sr = Math.round(origR * 0.08);
                const sg = Math.round(origG * 0.08);
                const sb = Math.round(origB * 0.08);
...
class GeneralTab {
    ...
    init() {
        ...
        // Grid Size Dropdown Logic
        const gridSizePresetSelect = document.querySelector('#set-grid-size-preset select');
        const customContainer = document.getElementById('grid-size-custom-container');
        if (gridSizePresetSelect) {
            gridSizePresetSelect.addEventListener('change', (e) => {
                const val = e.target.value;
                if (val === 'custom') {
                    customContainer.style.display = 'block';
                } else {
                    customContainer.style.display = 'none';
                    // Update the underlying slider and send the update
                    const numericVal = parseInt(val);
                    if (!isNaN(numericVal)) {
                        setSliderValue('[data-val="v-gs"]', numericVal);
                        this.getConfig().general_settings.grid_size = numericVal;
                        this.sendUpdate();
                    }
                }
            });
    ...
    updateUI(cfg) {
        ...
        const gsVal = gen.grid_size || 120;
        setSliderValue('[data-val="v-gs"]', gsVal);
        
        const gridSizePresetSelect = document.querySelector('#set-grid-size-preset select');
        const customContainer = document.getElementById('grid-size-custom-container');
        if (gridSizePresetSelect) {
            const predefined = [80, 120, 160, 200];
            if (predefined.includes(gsVal)) {
                setInputValue('#set-grid-size-preset select', gsVal.toString());
                if (customContainer) customContainer.style.display = 'none';
            } else {
                setInputValue('#set-grid-size-preset select', 'custom');
        ...
        const themeIntensity = gen.theme_intensity || (gen.folder_darkness !== undefined ? 
            (gen.folder_darkness === 'Light' ? 'Subtle' : 
             ...
             gen.folder_darkness === 'Pitch Black' ? 'Solid' : 'Balanced') 
            : 'Balanced');
