...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        pill.style.borderColor = color.replace(')', ',0.2)').replace('rgb', 'rgba');
        pill.querySelector('.pill-dot').style.background = color;
        pill.querySelector('.pill-dot').style.boxShadow = '0 0 8px ' + color;

        // Update sidebar active indicator & glow
...
const searchIndex = [
    ...
    { id: "set-show-grid-drag", name: "Show Grid on Drag", tab: "general", group: "Appearance" },
    { id: "set-anim-grid-color", name: "Animated Grid Color", tab: "general", group: "Appearance" },
    { id: "set-wave-ent", name: "Wave Entrance", tab: "general", group: "Appearance" },
    { id: "set-wave-color", name: "Wave Color Fade", tab: "general", group: "Appearance" },
    { id: "set-filter-preset", name: "Filter Preset", tab: "general", group: "Display Effects" },
    { id: "v-wi", name: "Warmth Intensity", tab: "general", group: "Display Effects" },
    ...
    { id: "set-media-art", name: "Art Style", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-vis", name: "Visualizer", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-mosaic", name: "Mosaic Style", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-mosaic-shape", name: "Mosaic Shape", tab: "halo", group: "Media Widget Settings" },
    { id: "v-media-strength", name: "Effect Strength", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-timeline", name: "Show Timeline", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-title", name: "Show Title", tab: "halo", group: "Media Widget Settings" },
    { id: "set-media-controls", name: "Show Controls", tab: "halo", group: "Media Widget Settings" },
    { id: "set-time-mode", name: "Clock Mode", tab: "halo", group: "Time Widget Settings" },
    { id: "set-time-24h", name: "24-Hour Format", tab: "halo", group: "Time Widget Settings" },
    { id: "set-time-date", name: "Show Date", tab: "halo", group: "Time Widget Settings" },
    { id: "set-time-seconds", name: "Show Seconds", tab: "halo", group: "Time Widget Settings" }
];
...
pillSearchInput.addEventListener('input', (e) => {
    ...
    currentResults = searchIndex.filter(item => item.name.toLowerCase().includes(q));
...
function renderHaloRadial() {
    ...
    if (wrap) {
        ...
        // Force SVG assets which are natively dark to invert if on Dark/Glass themes
        wrap.style.setProperty('--icon-filter', currentTheme === 'Light' ? 'none' : 'invert(1)');
    }
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (!isEmpty) {
            ...
            } else if (displayIcon && !displayIcon.includes('.svg')) {
                displayIcon = `../assets/${displayIcon}.svg`;
            }

            if (displayIcon.endsWith('.svg')) {
                const fo = document.createElementNS(ns, 'foreignObject');
                fo.setAttribute('x', labelPos.x - 12);
                fo.setAttribute('y', labelPos.y - 18);
                fo.setAttribute('width', '24');
                fo.setAttribute('height', '24');
                fo.setAttribute('style', 'pointer-events: none; overflow: visible;');
                const iconImg = document.createElement('img');
                iconImg.src = displayIcon;
                iconImg.style.cssText = 'width:24px; height:24px; pointer-events:none; filter:var(--icon-filter); display:block;';
                fo.appendChild(iconImg);
                g.appendChild(fo);
            } else {
...
window.updateHaloLayersFromConfig = function(menus) {
    ...
    haloLayers = Array.from({ length: 9 }, (_, i) => {
        if (menus[i] && menus[i].tools && menus[i].tools.length > 0) {
            return { slices: menus[i].tools.slice(0, MAX_SLICES).map(t => ({ icon: t.icon, name: t.label })) };
        }
...
function openCmdBank(sliceIndex) {
    ...
    grid.innerHTML = '';
    HALO_TOOLS.forEach(tool => {
        const item = document.createElement('div');
        item.className = 'halo-cmd-item';
        if (slice.name === tool.name) item.classList.add('active');
    ...
    rmBtn.style.display = slice.name ? 'flex' : 'none';
...
let hubSlots = [
    ...
    { type: 'media', settings: { art_style: 'Gaussian Blur', effect_strength: 25, visualizer: 'None', mosaic_style: 'Flat', mosaic_shape: 'Square', show_timeline: true, show_title: true, show_controls: true } },
...
function assignHubModule(index, type) {
    const defaultSettings = {
        pandora: {},
        media: { art_style: 'Gaussian Blur', effect_strength: 25, visualizer: 'None', mosaic_style: 'Flat', mosaic_shape: 'Square', show_timeline: true, show_title: true, show_controls: true },
    ...
    if (slot) {
        ...
        if (slot.type === 'pandora') {
            ...
            if (isMosaic) {
                html += `
                    <div class="control-row">
                        <span class="control-label">Mosaic Style</span>
                        <select id="setting-media-mosaicstyle">
                            <option value="Flat" ${slot.settings.mosaic_style === 'Flat' ? 'selected' : ''}>Flat</option>
                            <option value="Extrusion" ${slot.settings.mosaic_style === 'Extrusion' ? 'selected' : ''}>Extrusion</option>
                        </select>
                    </div>
                    <div class="control-row">
                        ...
                        <select id="setting-media-mosaicshape">
                            ...
                            <option value="Circles" ${slot.settings.mosaic_shape === 'Circles' ? 'selected' : ''}>Circles</option>
                            <option value="Hexagons" ${slot.settings.mosaic_shape === 'Hexagons' ? 'selected' : ''}>Hexagons</option>
                            <option value="Rounded" ${slot.settings.mosaic_shape === 'Rounded' ? 'selected' : ''}>Rounded</option>
                            <option value="Diamonds" ${slot.settings.mosaic_shape === 'Diamonds' ? 'selected' : ''}>Diamonds</option>
                        </select>
                    </div>
                `;
            }
            ...
            html += `
                ...
                <div class="control-row">
                    ...
                    <select id="setting-media-visualizer">
                        ...
                        ${isMosaic ? `
                        <option value="Voxel Wiggle" ${slot.settings.visualizer === 'Voxel Wiggle' ? 'selected' : ''}>Voxel Wiggle</option>
                        <option value="Size Pulsing" ${slot.settings.visualizer === 'Size Pulsing' ? 'selected' : ''}>Size Pulsing</option>
                        <option value="Brightness Strobing" ${slot.settings.visualizer === 'Brightness Strobing' ? 'selected' : ''}>Brightness Strobing</option>
                        ` : `
                        <option value="Breathing Blur" ${slot.settings.visualizer === 'Breathing Blur' ? 'selected' : ''}>Breathing Blur</option>
                        `}
                    </select>
                </div>
                <div class="control-row">
        ...
        content.querySelectorAll('select').forEach(sel => {
            sel.addEventListener('change', () => {
                ...
                if (sel.id === 'setting-media-artstyle') {
                    ...
                    if (val === '8-Bit Mosaic') {
                        slot.settings.visualizer = slot.settings.last_mosaic_vis || 'Voxel Wiggle';
                    } else {
                        slot.settings.visualizer = slot.settings.last_blur_vis || 'Breathing Blur';
                    }
                    openWidgetSettings(index); // Re-render to show/hide mosaic block style!
                }
                else if (sel.id === 'setting-media-mosaicstyle') slot.settings.mosaic_style = val;
